...
We just stratched the surface of what's possible with Searchkick, and
...
By default, everytime you change a record, if Searchkick is enabled
on that model, a reindex will be triggered for that record only.
While this is very useful, it can hurt stores perfomances.
...
To enabale this strategy, you can edit the directive that enabled
...
Another important aspect of the the automatic reindexing is how
...
For example, imagine if you only want to index products that has
...
`total_on_hand` checks all the product's variants stock levels via a
model called Spree::StockItem. This model contains all the information
about the remaining stock for a given variant in a given warehose.

:::

Nice and clean, but what if someone purchase that product and its stock
goes to 0?
...
You might be wondering why we are using this powerful Searchkick capabilities
...
Unfortunately, replacing the whole sercher with Searchkick properly requires us
...
made this how-to way more complex. We'll leave here an intial implementation idea,
...
As final step, we can tell Solidus to use this class with the proper configuration:
